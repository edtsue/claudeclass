// /api/roster — instructor adds/removes student names live. Gated by the edit cookie
// (cc_edit / EDIT_PASSWORD); writes with the Supabase secret key. Reads are public
// (the client reads the roster table directly with the publishable key).
...
export default async function handler(req, res) {
  ...
  try {
    if (action === 'add') {
      ...
      const max = await supa('roster?select=position&order=position.desc&limit=1', { method: 'GET', headers: { Prefer: 'count=none' } });
      let position = 0;
      if (max.ok) { const rows = await max.json(); position = (rows[0]?.position ?? -1) + 1; }
      const r = await supa('roster', { method: 'POST', body: JSON.stringify({ name, position }) });
    ...
    if (action === 'remove') {
      ...
      const r = await supa(`roster?id=eq.${id}`, { method: 'DELETE' });
      if (!r.ok) throw new Error(await r.text());
      return res.status(200).json({ ok: true });
    }
    return res.status(400).json({ error: 'unknown action' });
  } catch (e) { console.error('roster', e); return res.status(502).json({ error: 'save failed' }); }
}
